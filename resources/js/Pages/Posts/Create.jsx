...
export default function Create({ auth, categories }) {
    ...
    const handleAddToGallery = (items) => {
        ...
        const current = data.gallery_images.filter(img => typeof img === 'string');
        const merged = Array.from(new Set([...current, ...newPaths]));
        // Keep File objects too
        const files = data.gallery_images.filter(img => typeof img !== 'string');
        setData('gallery_images', [...merged, ...files]);
    };

    const handleRemoveGalleryItem = (item) => {
        setData('gallery_images', data.gallery_images.filter(img => img !== item));
    };

    const getCoverSrc = () => {
        if (!data.image) return null;
        if (typeof data.image === 'string') return resolveUrl(data.image);
        try { return URL.createObjectURL(data.image); } catch { return null; }
    };
        ...
        <AuthenticatedLayout
            ...
            header={
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        <h2 className="font-extrabold text-3xl tracking-tight text-theme-text leading-tight">New Article</h2>
                        <p className="text-sm text-theme-muted mt-1">Write and publish a new blog post.</p>
                    </div>
                    <button
                        type="button"
                        ...
                        <div className="flex gap-6 items-start">
                            ...
                            <div className="flex-1 space-y-6">
                                ...
                                <div className="bg-theme-card border border-theme-border/60 rounded-2xl p-6 space-y-4 shadow-theme-xs">
                                    ...
                                    <div>
                                        <InputLabel htmlFor="slug" value="Custom SEO Slug (optional)" className="text-sm font-bold text-theme-text mb-2" />
                                ...
                                <div className="bg-theme-card border border-theme-border/60 rounded-2xl shadow-theme-xs overflow-hidden">
                                    ...
                                    <div className="p-5 space-y-4">
                                        ...
                                        <label className="flex items-center gap-3 cursor-pointer">
                                            ...
                                            />
                                            <span className="text-sm font-semibold text-theme-text">Publish immediately</span>
                                        </label>
                                    ...
                                    <div className="p-4 space-y-3">
                                        {getCoverSrc() ? (
                                            <div className="rounded-xl overflow-hidden border border-theme-border/40 aspect-video">
                                                <img src={getCoverSrc()} alt="Featured cover" className="w-full h-full object-cover" />
                                            </div>
                                        ) : (
                                    ...
                                    <div className="px-5 py-3 bg-theme-accent/30 border-b border-theme-border/40 flex justify-between items-center">
                                        <h4 className="text-sm font-extrabold text-theme-text">
                                            ...
                                            {data.gallery_images.length > 0 && (
                                                <span className="ml-2 text-xs font-semibold text-theme-muted">({data.gallery_images.length})</span>
                                            )}
                                        </h4>
                                        {data.gallery_images.length > 0 && (
                                            <button type="button" onClick={() => setData('gallery_images', [])} className="text-xs text-red-400 hover:text-red-600 font-bold">Clear all</button>
                                        )}
                                    </div>
                                    <div className="p-4 space-y-3">
                                        {data.gallery_images.length > 0 ? (
                                        ...
                                        <label className="flex items-center gap-2 text-xs text-theme-muted hover:text-theme-text cursor-pointer transition">
                                            <input
                                                ...
                                                accept="image/*"
                                                multiple
                                                onChange={(e) => setData('gallery_images', [...data.gallery_images, ...Array.from(e.target.files)])}
                                            />
                                            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" /></svg>
                                            Or upload from computer
                                        </label>
